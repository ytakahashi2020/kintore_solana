...
pub mod bank_simulator {
    ...
    pub fn add_interest(ctx: Context<AddInterest>, _thread_id: Vec<u8>) -> Result<()> {
        let now = Clock::get().unwrap()./* ①時刻取得 */;

        let bank_account = &mut ctx.accounts.bank_account;
        bank_account.updated_at = now;

        let elapsed_time = (now - bank_account.created_at) /* ②型変換 */ f64;
        let minutes = elapsed_time / 60.0;
        let accumulated_value = bank_account.balance * (1.0 + (MINUTE_INTEREST))./* ③べき乗 */(minutes);
...
pub struct Initialize<'info> {
    ...
    #[account(
        init,
        payer = holder,
        seeds = /* ⑤BANK_ACCOUNT_SEEDとthiread_idのシード */,
        bump,
        space = 8 + /* ⑥メモリサイズの取得 */::<BankAccount>(),
    )]
    pub bank_account: Account<'info, BankAccount>,

    #[account(
        mut, 
        address = /* ⑦公開鍵の取得*/(/* ⑦公開鍵*/, /* ⑧スレッドID*/)
    )]
    pub thread: /* ⑨システムアカウントの型*/,

    #[account(
        /* 10THREAD_AUTHORITY_SEEDのシード*/
        bump
    )]
    pub thread_authority: /* 11システムアカウントの型*/,

    #[account(
        address = /* 12ClockworkSDKのID*/
    )]
    pub clockwork_program: /* 13clockwork_sdkのThreadProgramのプログラム*/,

    pub system_program: /* 14 system program*/,
}
