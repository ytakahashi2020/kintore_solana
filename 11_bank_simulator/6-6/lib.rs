...
pub mod bank_simulator {
    ...
    pub fn deposit(ctx: Context<UpdateBalance>, _thread_id: Vec<u8>, amount: f64) -> Result<()> {
        if amount < 0.0 {
            return /* ①エラー */(/* ②エラー */(/* ③エラーコード */));
        };

        let bank_account = /* ④変更可能 */ /* ⑤アカウント */;
...
#[error_code]
pub enum ErrorCode {
    /* 11 "Amount must be greater than zero"というメッセージ*/
    AmountTooSmall,

    /* 12 "Withdraw amount cannot be less than deposit"というメッセージ*/
    AmountTooBig,
}
